...
import (
	...
	"github.com/chindeo/rtsp2hls/utils"
...
func (client *RTSPClient) requestStream(timeout time.Duration) (err error) {
	...
	if err != nil {
		if resp != nil {
			...
			if len(Authorization) > 0 {
				...
				// An OPTIONS request returns the request types the server will accept.
				_, err = client.Request("OPTIONS", headers)
				if err != nil {
					return err
				}
			}
	...
	_, err = client.Request("PLAY", headers)
	if err != nil {
		return err
	}
	return nil
...
func (client *RTSPClient) rtpHandler() {
	for _, h := range client.RTPHandles {
		h(<-ClientRTPPackCh)
	}
}
